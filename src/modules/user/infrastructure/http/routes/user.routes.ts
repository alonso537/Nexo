...
import {
  IsAdmin,
  IsSuperAdmin,
} from '../../../../../shared/infrastructure/http/express/middleware/authorize.middleware';
...
import { GetUserBySlugSchema } from '../../../application/dto/getUserBySlug.dto';
...
userRoutes.get('/', authenticate, IsAdmin, validate(FilterUsersSchema), user.getAllUsers);
userRoutes.get('/:username', authenticate, validate(GetUserBySlugSchema), user.getUserBySlug);
...
userRoutes.patch(
  ...
  authenticate,
  IsAdmin,
  validate(ChangeRoleLimitedSchema),
  user.changeRole,
);
userRoutes.patch(
  '/:id/role/admin',
...
userRoutes.delete('/avatar', authenticate, user.deleteAvatar);

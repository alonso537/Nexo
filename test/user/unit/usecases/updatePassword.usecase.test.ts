...
describe('UpdatePasswordUseCase', () => {
  ...
  describe('execute()', () => {
    it('should update the password when current password is correct', async () => {
      ...
      await usecase.execute(user.toPersistence().id, {currentPassword: '123456789askf', newPassword: 'nuevaContrasena123'})
    ...
    it('should increment tokenVersion after password change', async () => {
      ...
      const result = await usecase.execute(user.toPersistence().id, {currentPassword: '123456789askf', newPassword: 'nuevaContrasena123'})
    ...
    it('should throw when the current password is incorrect', async () => {
      ...
      await expect(usecase.execute(user.toPersistence().id, {currentPassword: 'wrongPassword', newPassword: 'nuevaContrasena123'})).rejects.toThrow(AppError);
    });
    it('should throw when the user is not found', async () => {
      ...
      await expect(usecase.execute('no-valid-id', {currentPassword: '123456789askf', newPassword: 'nuevaContrasena123'})).rejects.toThrow(AppError);
    });
  });
});

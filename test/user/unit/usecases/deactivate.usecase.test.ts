...
import { DeactivateUsecase } from '../../../../src/modules/user/application/usecase/deactivate.usecase';
...
describe('DeactivateUsecase', () => {
  let usecase: DeactivateUsecase;

  beforeEach(() => {
    vi.clearAllMocks();
    usecase = new DeactivateUsecase(mockRepository, mockCache);
  });
  ...
  describe('execute()', () => {
    it('should set the user status to INACTIVE', async () => {
      ...
      expect(savedUser.toPersistence().status).toBe('INACTIVE');
    });

    it('should invalidate the cache after deactivating', async () => {
      const user = createActiveUser();

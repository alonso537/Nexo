...
describe('UpdateNameUseCase', () => {
  ...
  describe('execute()', () => {
    ...
    it('should throw when name is invalid (too short, special chars)', async () => {
      ...
      await expect(() => usecase.execute(user.toPersistence().id, { name:'J' })).rejects.toThrow(AppError);
      await expect(() => usecase.execute(user.toPersistence().id, { name:'John@' })).rejects.toThrow(AppError);
